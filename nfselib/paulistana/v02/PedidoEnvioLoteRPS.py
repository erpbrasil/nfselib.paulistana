...
#   ('-f', '')
#   ('-o', '/home/luisotavio/Documentos/Projects/produto_12/src/nfselib.paulistana/paulistanalib/v02/PedidoEnvioLoteRPS.py')
#
# Command line arguments:
#   /tmp/generated/schemas/paulistana/v02/PedidoEnvioLoteRPS_v01.xsd
#
# Command line:
#   /home/luisotavio/Documentos/erpbrasil/bin/generateDS.py --no-namespace-defs --no-dates --member-specs="list" --use-getter-setter="none" -f -o "/home/luisotavio/Documentos/Projects/produto_12/src/nfselib.paulistana/paulistanalib/v02/PedidoEnvioLoteRPS.py" /tmp/generated/schemas/paulistana/v02/PedidoEnvioLoteRPS_v01.xsd
#
...
class PedidoEnvioLoteRPS(GeneratedsSuper):
    """Schema utilizado para PEDIDO de envio de lote de RPS.Este Schema XML é
    utilizado pelos prestadores de serviços para substituição em lote de
    RPS por NFS-e."""
    ...
    def export(self, outfile, level, namespaceprefix_='', namespacedef_='', name_='PedidoEnvioLoteRPS', pretty_print=True):
        ...
        self.exportAttributes(outfile, level, already_processed, namespaceprefix_, name_='PedidoEnvioLoteRPS')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespaceprefix_, namespacedef_, name_='PedidoEnvioLoteRPS', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespaceprefix_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespaceprefix_='', name_='PedidoEnvioLoteRPS'):
        pass
    def exportChildren(self, outfile, level, namespaceprefix_='', namespacedef_='', name_='PedidoEnvioLoteRPS', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
...
class tpRPS(GeneratedsSuper):
    ...
    def exportChildren(self, outfile, level, namespaceprefix_='tipos:', namespacedef_='', name_='tpRPS', fromsubclass_=False, pretty_print=True):
        ...
        if self.Assinatura is not None:
            namespaceprefix_ = self.Assinatura_nsprefix_ + ':' if (UseCapturedNS_ and self.Assinatura_nsprefix_) else ''
            showIndent(outfile, level, pretty_print)
            outfile.write('<%sAssinatura>%s</%sAssinatura>%s' % (namespaceprefix_ , self.gds_format_string(self.Assinatura, input_name='Assinatura'), namespaceprefix_ , eol_))
        if self.ChaveRPS is not None:
            namespaceprefix_ = self.ChaveRPS_nsprefix_ + ':' if (UseCapturedNS_ and self.ChaveRPS_nsprefix_) else ''
            self.ChaveRPS.export(outfile, level, namespaceprefix_, namespacedef_='', name_='ChaveRPS', pretty_print=pretty_print)
        if self.TipoRPS is not None:
    ...
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False, gds_collector_=None):
        if nodeName_ == 'Assinatura':
            ...
            if sval_ is not None:
                ...
                # except (TypeError, ValueError) as exp:
                #     raise_parse_error(child_, 'requires base64 encoded string: %s' % exp)
                bval_ = self.gds_validate_base64(sval_, node, 'Assinatura')
            else:
                bval_ = None
            self.Assinatura = bval_
            self.Assinatura_nsprefix_ = child_.prefix
